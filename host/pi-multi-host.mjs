...
async function buildRuntimeHost({ cwd, sessionFile }) {
  ...
  // NOTE: services (resource loader / extension runtime) are built PER SESSION
  // on purpose. A per-cwd services cache was tried — it made 2nd+ opens ~21ms,
  // but Pi's extension runtime (MCP) holds per-creation state and every later
  // session failed with "MCP initialization failed: stale ctx". Only the
  // ModelRuntime is shared; the prewarmed draft session hides the boot cost
  // where it matters (New Chat).
